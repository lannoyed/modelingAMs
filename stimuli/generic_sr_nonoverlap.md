...
|---|---|---|
| `tnov` (ns) | the repo `` `timescale 1ns/1ps`` precision floor (`tnov = 0.001`, ~1 ps), a realistic value (`tnov = 1`, ~1 ns), and up toward a half period | dead-time width should scale with `tnov`; below ~1 ps the delay rounds toward zero (no dead time — a real floor from the fixed timescale), the large corner exposes pulse swallowing (sequence 3) |
